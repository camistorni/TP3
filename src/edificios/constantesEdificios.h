#ifndef CONSTANTESEDIFICIOS_H
#define CONSTANTESEDIFICIOS_H

#include <iostream>

const char CARACTER_ASERRADERO      = 'A';
const char CARACTER_ESCUELA         = 'E';
const char CARACTER_FABRICA         = 'F';
const char CARACTER_MINA            = 'M';
const char CARACTER_MINA_ORO        = 'G';
const char CARACTER_OBELISCO        = 'O';
const char CARACTER_plantaElectrica = 'P';

const std::string ASERRADERO        = "aserradero";
const std::string ESCUELA           = "escuela";
const std::string FABRICA           = "fabrica";
const std::string MINA              = "mina";
const std::string MINA_ORO          = "mina de oro";
const std::string OBELISCO          = "obelisco";
const std::string PLANTA_ELECTRICA  = "planta eléctrica";

#define PRODUCCION_MINA             15
#define PRODUCCION_ASERRADERO       25
#define PRODUCCION_FABRICA          40
#define PRODUCCION_ESCUELA          25
#define PRODUCCION_PLANTA_ELECTRICA 15
#define PRODUCCION_MINA_ORO         50



#endif