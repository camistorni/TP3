...
void Juego::listarEdificiosConstruidos(){
	...
	int jugador = juego -> obtenerJugadorActivo();
	for(int i = 0; i < mapa -> obtenerCantidadFilas(); i++) {
		for(int j = 0; j < mapa -> obtenerCantidadColumnas(); j++){
			if(juego -> obtenerMapa() -> obtenerCasillero(i, j) -> obtenerTipo() == TERRENO){
				if(static_cast<CasilleroConstruible*>(juego -> obtenerMapa() -> obtenerCasillero(i, j)) -> obtenerPropietarioEdificio() == jugador){
					cout << static_cast<CasilleroConstruible*>(juego -> obtenerMapa() -> obtenerCasillero(i, j)) -> obtenerEdificio() << setw(16);
					cout << 
				}
			}
		}
