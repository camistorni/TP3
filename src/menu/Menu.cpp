...
void Menu::moverseAUnaCoordenada(Juego *juego) {
	...
	caminoDisponible = juego -> mostrarCaminoMinimo(posicionActual, coordenada, &energiaGastada);
	cout << endl;
	if(caminoDisponible){
		if(juego -> obtenerJugador() -> obtenerEnergia() >= energiaGastada){
			...
			int opcion = 0;
			cout << "No tiene suficiente energía para completar esta acción. Puede elegir otra coordenada(1) o volver al menú(cualquier tecla)" << endl;
